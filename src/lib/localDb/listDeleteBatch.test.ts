...
describe('planListDeleteBatches', () => {
  it('deletes the list in the same batch as the final membership chunk', () => {
    expect(planListDeleteBatches(0)).toEqual([{ startIndex: 0, endIndex: 0, deleteList: true }]);

    expect(planListDeleteBatches(1)).toEqual([{ startIndex: 0, endIndex: 1, deleteList: true }]);
  });
