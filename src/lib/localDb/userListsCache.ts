...
function upsertUserListRows(db: Database, userId: string, lists: PlaceList[], updatedAt: number): void {
...
/** Replace dashboard rows so deleted/unsaved lists do not linger locally. */
...
export async function removeCachedUserListRow(userId: string, listId: string): Promise<void> {
  await runWriteAsync((db) => {
