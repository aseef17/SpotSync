  ...
  const fetchWithProxy = async (targetUrl: string): Promise<string> => {
       ...
       for (const proxy of corsProxies) {
           try {
               ...
               if (!text || text.length < 500) throw new Error('Response too short to be a valid Maps page');
               
               // Bot/Consent/Landing Page Detection
               const isBlocked = text.includes('detected unusual traffic') || 
               ...
               const isGoogle = text.includes('google.com') || text.includes('window.APP_INITIALIZATION_STATE');
...
function parseGoogleMapsHtml(html: string): ParsedGoogleMapsList {
  ...
  let listName = 'Imported Google Maps List';
  ...
  const scavengeArray = (arr: unknown[], parentPath: string) => {
      ...
      if (metadata) {
          const foundAddress = metadata.find((sub): sub is string => typeof sub === 'string' && sub.includes(',') && sub.length > 8);
          address = foundAddress || (typeof metadata[4] === 'string' ? metadata[4] : '') || (typeof metadata[2] === 'string' ? metadata[2] : '') || '';
          const coords = metadata.find((sub): sub is number[] => 
             Array.isArray(sub) && (
                  (sub.length >= 2 && typeof sub[0] === 'number' && typeof sub[1] === 'number' && sub[0] !== 0) ||
                  (sub.length >= 4 && typeof sub[2] === 'number' && typeof sub[3] === 'number' && sub[2] !== 0)
              )
          );
          if (coords) {
              if (typeof coords[0] === 'number' && coords[0] !== 0) { lat = coords[0]; lng = coords[1]; }
              else { lat = coords[2]; lng = coords[3]; }
          }
          cid = metadata.find((sub): sub is string => typeof sub === 'string' && sub.startsWith('0x')) || (metadata[8] as string[])?.[0];
      }
  ...
  while (true) {
    ...
    if (isQuoted) {
        ...
        for (let i = start; i < html.length; i++) {
            ...
            else if (char === '"') { end = i; foundEnd = true; break; }
        }
        if (foundEnd) content = html.substring(start, end).replace(/\\"/g, '"').replace(/\\\\/g, '\\');
    } else {
        let open = 0;
        let inString = false;
        let escape = false;
            ...
            for (let i = start; i < html.length; i++) {
                ...
                if (inString) {
                    ...
                    if (char === '"') inString = true;
                        ...
                        open--;
                        if (open === 0) { end = i + 1; foundEnd = true; break; }
                    }
                }
            }
        }
    ...
    if (foundEnd && content.length > 20) {
        ...
        } catch { /* ignore parsing errors of sub-blocks */ }
    }
  ...
  for (const regex of globalRegexes) {
      ...
      if (match && match[1]) {
          try {
              const cleaned = match[1].replace(/([{,])\s*(\w+):/g, '$1"$2":').replace(/:\s*'([^']*)'/g, ':"$1"').replace(/,\s*([}\]])/g, '$1');
              deepSearch(JSON.parse(cleaned), 'root');
          } catch { deepSearch(match[1], 'root'); }
      }
  }
  ...
  while (pos < html.length) {
      ...
      for (let i = bracketMatch; i < Math.min(bracketMatch + 500000, html.length); i++) {
          if (html[i] === '[') open++;
          else if (html[i] === ']') { open--; if (open === 0) { end = i + 1; break; } }
      }
      if (end !== -1) {
          try { deepSearch(JSON.parse(html.substring(bracketMatch, end).replace(/,\s*([}\]])/g, '$1')), 'root'); } catch { /* ignore */ }
          pos = end;
