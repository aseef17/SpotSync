    ...
    return (
        ...
        >
            <div className="flex justify-between items-start mb-1">
                <h3 className={`text-base font-semibold ${themeColors.text.primary} line-clamp-1 flex-1 mr-2`}>
                    {place.name}
                </h3>
                {place.status !== 'not_visited' && (
                    <span className={`flex-shrink-0 px-2 py-0.5 rounded text-[10px] font-bold uppercase tracking-wider ${place.status === 'visited'
            ...
            {photos.length > 0 && (
                <div className="flex gap-2 overflow-x-auto pb-2 -mx-4 px-4 scrollbar-hide mb-3">
                    {photos.slice(0, 5).map((photo: string, i: number) => (
                        <div key={i} className="flex-shrink-0 w-32 h-32 rounded-lg overflow-hidden bg-gray-100 dark:bg-gray-800">
                            <img
            ...
            {place.notes && (
                <p className={`text-sm ${themeColors.text.primary} mb-2 line-clamp-2`}>
                    {place.notes}
                </p>
            )}
