...
import {
    ...
    Navigation, Play, Share2, Edit3, Trash2,
    ChevronDown, ChevronUp, Camera, Check, Utensils, Plus
} from 'lucide-react';
...
export const MobilePlaceDetailHeader: React.FC<MobilePlaceDetailHeaderProps> = ({
    ...
    const currentStatusValue = useMemo(() => place.status === 'custom' ? place.customStatus : place.status, [place]);

    const currentLabel = useMemo(() => statusOptions.find(o => o.value === currentStatusValue)?.label || 'Set Status', [currentStatusValue, statusOptions]);
        ...
        >
            ...
            <div className="flex justify-between items-start">
                <div className="flex-1 mr-4">
                    ...
                    <div className={`flex flex-wrap items-center gap-1.5 text-sm ${themeColors.text.secondary} mt-1`}>
                    ...
                    <div className="fixed inset-0 z-[9999] isolate">
                        ...
                        >
                            <div className="p-4 border-b border-gray-100 dark:border-gray-800">
                                <h3 className={`text-center font-semibold ${themeColors.text.primary}`}>Set Status</h3>
                            </div>
        ...
        >
            ...
            <div>
                ...
                {photos.length > 0 ? (
                    <div className="flex gap-2 overflow-x-auto pb-2 -mx-4 px-4 scrollbar-hide">
                        {photos.map((photo, i) => (
                            <div key={i} className="flex-shrink-0 w-60 h-40 rounded-xl overflow-hidden bg-gray-100 dark:bg-gray-800 shadow-sm relative group">
                                <img
            ...
            <div className="space-y-4">
                ...
                <div className="flex items-start gap-3">
                    ...
                    <span className={`text-sm ${themeColors.text.secondary} select-all`}>{place.address}</span>
                </div>
                ...
                {place.openingHours && place.openingHours.length > 0 && (
                    <div className="flex items-start gap-3">
                        ...
                        <div className="flex-1">
                            ...
                            >
                                <span className={place.openNow ? 'text-green-600 font-medium' : 'text-red-600 font-medium'}>
                                    {place.openNow ? 'Open' : 'Closed'}
                                </span>
                                <span className={`${themeColors.text.secondary}`}>
                                    · {place.openingHours[(new Date().getDay() + 6) % 7].split(':')[0]}
                                </span>
                                {isHoursExpanded ? <ChevronUp className="h-4 w-4 ml-auto" /> : <ChevronDown className="h-4 w-4 ml-auto" />}
                            </button>
                            ...
                            <AnimatePresence>
                                {isHoursExpanded && (
                                    ...
                                    >
                                        <div className={`mt-2 space-y-1 text-sm ${themeColors.text.secondary} border-l-2 border-gray-200 dark:border-gray-700 pl-3 ml-0.5`}>
                                            {place.openingHours.map((hour, idx) => {
                                                ...
                                                return (
                                                    <div key={idx} className={`grid grid-cols-[100px_1fr] ${isToday ? `font-medium ${themeColors.text.primary}` : ''}`}>
                                                        <span>{day}</span>
                                                        <span>{time}</span>
                                                    </div>
                                                );
