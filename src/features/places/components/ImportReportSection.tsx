...
export const ImportReportSection: React.FC<ImportReportSectionProps> = ({
    ...
    return (
        <div className="border rounded-md bg-white dark:bg-gray-800 overflow-hidden">
            <button
                onClick={onToggle}
                className={`w-full flex items-center justify-between p-3 text-sm font-medium transition-colors ${type === 'failed'
            ...
            >
                <div className="flex items-center gap-2">
                    {type === 'failed' ? <AlertCircle className="h-4 w-4" /> : <AlertCircle className="h-4 w-4" />}
                    <span>{title} ({places.length})</span>
                </div>
            ...
            {isOpen && (
                <div className="divide-y dark:divide-gray-700 max-h-60 overflow-y-auto">
                    {places.map((place, i) => (
                        <div key={i} className="flex items-center gap-3 p-3 hover:bg-gray-50 dark:hover:bg-gray-700/50 transition-colors">
                            {/* Image / Icon */}
                            <div
                                ...
                                style={place.photoUrls?.[0] ? { backgroundImage: `url(${GoogleMapsService.getPhotoUrl(place.photoUrls[0])})` } : undefined}
                            >
                            ...
                            <div className="min-w-0 flex-1">
                                ...
                                {type === 'failed' && (
                                    <p className="text-xs text-red-500 mt-0.5">Import Error</p>
                                )}
                            </div>
                        </div>
                    ))}
