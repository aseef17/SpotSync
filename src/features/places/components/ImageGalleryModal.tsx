...
export const ImageGalleryModal: React.FC<ImageGalleryModalProps> = ({
    ...
    return (
        <div className={`fixed inset-0 z-[60] ${themeColors.background.modalOverlay} flex flex-col items-center justify-center`}>
            {/* Header */}
            ...
            <div className={`w-full ${themeColors.background.card} border-t ${themeColors.border.default} p-4 backdrop-blur-sm overflow-x-auto`}>
                <div className="flex justify-center gap-2 min-w-min mx-auto">
                    {images.map((img, idx) => (
                        <button
                            key={idx}
                            onClick={() => setCurrentIndex(idx)}
                            className={`relative flex-shrink-0 w-16 h-16 rounded-md overflow-hidden border-2 transition-all ${idx === currentIndex
