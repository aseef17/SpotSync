...
export const Dashboard: React.FunctionComponent = () => {
  ...
  const [listCacheProbe, setListCacheProbe] = useState<{
    userId: string;
    hadCache: boolean;
  } | null>(null);
  ...
  useEffect(() => {
    ...
    let cancelled = false;
    void listRepository.getForUser(userId).then((cachedLists) => {
      if (!cancelled) {
        setListCacheProbe({ userId, hadCache: cachedLists.length > 0 });
      }
    });
  ...
  const hadListCacheInitially =
    userId && listCacheProbe?.userId === userId ? listCacheProbe.hadCache : null;
