...
export const Settings: React.FunctionComponent = () => {
  ...
  const [hadProfileCacheInitially, setHadProfileCacheInitially] = useState<boolean | null>(null);

  useEffect(() => {
    if (!firebaseUser?.uid) {
      setHadProfileCacheInitially(null);
      return;
    }

    void readUserProfileFromCache(firebaseUser.uid).then((profile) => {
      setHadProfileCacheInitially(!!profile);
    });
  }, [firebaseUser?.uid]);
