...
if (missing.length > 0) {
  console.warn(`[Generate SW] Warning: Missing environment variables for Service Worker generation: ${missing.join(', ')}`);
...
try {
  ...
  content = content.replace(/'YOUR_PROJECT_ID.firebaseapp.com'/g, `'${env.VITE_FIREBASE_AUTH_DOMAIN}'`);
  content = content.replace(/'YOUR_PROJECT_ID.firebasestorage.app'/g, `'${env.VITE_FIREBASE_STORAGE_BUCKET}'`); // Note: bucket usually matches this pattern but let's be safe
  content = content.replace(/'YOUR_MESSAGING_SENDER_ID'/g, `'${env.VITE_FIREBASE_MESSAGING_SENDER_ID}'`);
